...
<head>
<meta charset="utf-8">
<title>ข้อมูลสถานที่ฝึกงานที่สนใจคุณ</title>
		...
		<section class="p-4 my-container">
			...
			<div class="table-container table-responsive">
				<div class="row abovetable ">
					<h4>ข้อมูลสถานที่ฝึกงานที่สนใจคุณ</h4>
				</div>
				...
				<table class="table  table-striped text-center"
					...
					<thead class="table-dark">
						<tr>
							<th>ชื่อบริษัทที่สนใจนิสิต/นักศึกษาเข้าฝึกงาน</th>
							<th>ที่ตั้ง</th>
							<th>สถานะ</th>
							<th>เวลาที่บันทึก</th>
							<th></th>
				...
				<div
					...
					<div class="modal-content" id="update">
						<div class="modal-header">
							<h5 class="modal-title">ข้อมูลสถานที่ฝึกงานที่สนใจคุณ</h5>
						</div>
...
$(function() {
	axios({
		  method: "post",
		  url: "studentRequestDataForRole1",
			...
			$.each(response.data, function(i, data) {
				    ...
				    newCell0.innerHTML = "<div class='d-flex '><div class='ms-3'><p class='fw-bold mb-1'>"+response.data[i].name+"</p><p class='text-muted mb-0'>"+response.data[i].internship_email+"</p></div></div>"
				    ...
				    newCell4.innerHTML = "<button type='button' onClick='ShowInternshipModal(\""+ response.data[i].name +"\",\""+ response.data[i].internship_email +"\",\""+ response.data[i].phone +"\",\""+ response.data[i].recive_total +"\",\""+ response.data[i].type +"\",\""+ response.data[i].status +"\",\""+ response.data[i].time_reg +"\",\""+ response.data[i].internship_id +"\",\""+ response.data[i].address1 +"\",\""+ response.data[i].address2 +"\",\""+ response.data[i].address3 +"\",\""+ response.data[i].address4 +"\",\""+ response.data[i].address5 +"\")' class='btn btn-warning' data-bs-toggle='modal' data-bs-target='#internshipModal'>ดูรายละเอียด</button>"
				    newCell5.innerHTML = "<button id='confirm' onClick='manegementStatus("+response.data[i].student_place_of_internship_id+", 1,\""+ response.data[i].teacher_email +"\",\""+ response.data[i].internship_email +"\",\""+ response.data[i].internship_id +"\")' class='btn btn-success'>ยืนยัน</button><button onClick='manegementStatus("+response.data[i].student_place_of_internship_id+", 0,\""+ response.data[i].teacher_email +"\",\""+ response.data[i].internship_email +"\",\""+ response.data[i].internship_id +"\")' class='btn btn-danger' id='cancel'>ปฏิเสธ</button>"
...
function manegementStatus(std_internship_id, status, teacher_email, internship_email, internship_id) {
	...
	const data = $.getJSON('https://api.ipify.org?format=json', function(data){
		...
		if(std_internship_id != '' && teacher_email != '' && internship_email != '' && internship_id != '') {
			
				
			axios({
				  method: "post",
				  url: "managementInternshipAndUserStatus",
				  data: 'std_internship_id=' + std_internship_id +'&status=' +status +'&ip='+ip +'&teacher_email='+teacher_email+'&internship_email='+internship_email+'&internship_id='+internship_id,
				}).then(function (response) {
