          ...
          <div class="modal-dialog modal-xl modal-fullscreen-lg-down modal-dialog-centered">
            <div class="modal-content">
              <div class="modal-header">
                <h5 id="notifyName" class="modal-title">รายละเอียดสถานที่ทำงาน</h5>
              </div>
...
$(function() {
	...
	function showModal(name, email, type, time_reg, address, phone, recive_total, oversea_id, user_id, province) {
		
		axios({
			  method: "post",
			  url: "findSkillByOverseas",
			  data: 'overseas_id=' +oversea_id,
			}).then(function (response) {
		...
		document.getElementById('overseaId').value = oversea_id
	...
	function sendData() {
		...
		const data = $.getJSON('https://api.ipify.org?format=json', function(data){
			...
			ip = data.ip
			var oversea_id = document.getElementById('overseaId').value
				
			if(oversea_id != '' && ip != '') {
					
				axios({
					  method: "post",
					  url: "sendDataToOverseas",
					  data: 'overseas_id=' + oversea_id +'&ip=' + ip,
					}).then(function (response) {
	...
	$('#zone').change(function() {
		...
		if(zone_value != '' && zone_value != '0') {
			...
			axios({
				  method: "post",
				  url: "findOverseasWorkPlaceByZone",
