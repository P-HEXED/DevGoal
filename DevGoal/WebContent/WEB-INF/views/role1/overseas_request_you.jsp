...
<head>
<meta charset="utf-8">
<title>ข้อมูลสถานที่ทำงานที่สนใจคุณ</title>
		...
		<section class="p-4 my-container">
			...
			<div class="table-container table-responsive">
				<div class="row abovetable ">
					<h4>ข้อมูลสถานที่ทำงานที่สนใจคุณ</h4>
				</div>
				...
				<table class="table  table-striped text-center"
					...
					<thead class="table-dark">
						<tr>
							<th>ชื่อบริษัทที่สนใจนิสิต/นักศึกษาเข้าทำงาน</th>
							<th>ที่ตั้ง</th>
							<th>สถานะ</th>
							<th>เวลาที่บันทึก</th>
							<th></th>
				...
				<div
					...
					<div class="modal-content" id="update">
						<div class="modal-header">
							<h5 class="modal-title">ข้อมูลสถานที่ทำงานที่สนใจคุณ</h5>
						</div>
											...
											<div class="col-sm-12 col-md-6 col-lg-4">
												<div class="form-floating ">
													<input type="text" class="form-control"
														...
														for="name">ชื่อสถานที่ทำงาน</label>
												</div>
											</div>
...
$(function() {
	axios({
		  method: "post",
		  url: "overseasRequestStudentData",
			...
			$.each(response.data, function(i, data) {
				    ...
				    newCell0.innerHTML = "<div class='d-flex '><div class='ms-3'><p class='fw-bold mb-1'>"+response.data[i].name+"</p><p class='text-muted mb-0'>"+response.data[i].overseas_email+"</p></div></div>"
				    newCell1.innerHTML = "<p>"+response.data[i].type+"</p>"
				    
				    var cell2 = ''
					    
				    if(response.data[i].status == "ไม่ตอบรับ") {
				    	
				    	cell2 = "<span class='badge badge-danger rounded-pill d-inline'>"+response.data[i].status+"</span>"
				    	
				    }else if(response.data[i].status == "ตอบรับแล้ว") {
				    ...
				    newCell4.innerHTML = "<button type='button' onClick='ShowInternshipModal(\""+ response.data[i].name +"\",\""+ response.data[i].overseas_email +"\",\""+ response.data[i].phone +"\",\""+ response.data[i].recive_total +"\",\""+ response.data[i].type +"\",\""+ response.data[i].status +"\",\""+ response.data[i].time_reg +"\",\""+ response.data[i].overseas_id +"\",\""+ response.data[i].address1 +"\",\""+ response.data[i].address2 +"\",\""+ response.data[i].address3 +"\",\""+ response.data[i].address4 +"\",\""+ response.data[i].address5 +"\")' class='btn btn-warning' data-bs-toggle='modal' data-bs-target='#internshipModal'>ดูรายละเอียด</button>"
				    newCell5.innerHTML = "<button id='confirm' onClick='manegementStatus("+response.data[i].student_overseas_work_place_id+", 1,\""+ response.data[i].employer_email +"\",\""+ response.data[i].overseas_email +"\",\""+ response.data[i].overseas_id +"\")' class='btn btn-success'>ยืนยัน</button><button onClick='manegementStatus("+response.data[i].student_overseas_work_place_id+", 0,\""+ response.data[i].employer_email +"\",\""+ response.data[i].overseas_email +"\",\""+ response.data[i].overseas_id +"\")' class='btn btn-danger' id='cancel'>ปฏิเสธ</button>"
...
function ShowInternshipModal(name, email, phone, receive, type, status, time_reg, overseas_id, address1, address2, address3, address4, address5) {
	...
	document.getElementById('internshipIdModal').value = overseas_id
...
function manegementStatus(std_overseas_id, status, employer_email, overseas_email, overseas_id) {
	...
	const data = $.getJSON('https://api.ipify.org?format=json', function(data){
		...
		if(std_overseas_id != '' && employer_email != '' && overseas_email != '' && overseas_id != '') {
				
			axios({
				  method: "post",
				  url: "managementOverseasAndUserStatus",
				  data: 'std_overseas_id=' + std_overseas_id +'&status=' +status +'&ip='+ip +'&employer_email='+employer_email+'&overseas_email='+overseas_email+'&overseas_id='+overseas_id,
				}).then(function (response) {
