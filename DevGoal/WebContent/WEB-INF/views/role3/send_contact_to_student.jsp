					...
					<div class="modal-content">
						...
						<div class="modal-body">
							...
							<div class="row">
								...
								<div class="col col-sm-12 col-md-12 col-lg-3">
									<label for="comment">สถานที่ทำงาน</label> <input
										...
										<div class="col-lg-12">
											<form>
												...
												<button type="button" onClick="insertPlaceOfInternship()"
													class="btn btn-primary" style="float: right;">ส่งข้อมูลการติดต่อให้นิสิต/นักศึกษา</button>
											</form>
										</div>
									</div>
								</div>
...
$(function() {
	...
	axios({
		  ...
		  url: "findOverseasWorkPlaceChoice",
			...
			$.each(response.data, function(i, data) {
             	$('#overseas_data').append($('<option value="' + response.data[i].overseas_work_place_id + '">' + response.data[i].name + '</option>'));
         });			 
	...
	function ShowUserData(profile_image, firstname, lastname, gender, birthday, university_name, faculty_name, course_name, user_id, province, overseas_name, email, phone) {
		...
		document.getElementById('userOverseas').value = overseas_name
	...
	$('#overseas_data').change(function() {
		...
		var overseas = e.options[e.selectedIndex].value;
		
		if(overseas != '') {
			
			axios({
				  method: "post",
				  url: "overseasAndStudentMatchingData",
				  data: "overseas="+overseas,
					...
					$.each(response.data, function(i, data) {
					    ...
					    newCell.innerHTML = "<input id='checkBox' type='checkbox' value='"+response.data[i].student_overseas_work_place_id+"'/>"
					    ...
					    newCell3.innerHTML = "<button type='button' onClick='ShowUserData(\""+response.data[i].profile_image+"\",\""+ response.data[i].firstname+"\",\""+ response.data[i].lastname+"\",\""+ response.data[i].gender+"\",\""+ response.data[i].birthday+"\",\""+ response.data[i].university_name+"\",\""+ response.data[i].faculty_name+"\",\""+ response.data[i].course_name+"\",\""+response.data[i].user_id+"\",\""+response.data[i].province+"\",\""+response.data[i].overseas_name+"\",\""+response.data[i].email+"\",\""+response.data[i].phone+"\")' class='btn btn-info' data-bs-toggle='modal' data-bs-target='#info'>ดูข้อมูลส่วนตัว</button>"
...
function insertPlaceOfInternship() {
	...
	$(':button').prop('disabled', true);
		...
		const data = $.getJSON('https://api.ipify.org?format=json', function(data){
			...
			var ip = data.ip
			var std_overseas_id = getStdOverseasId()
			
			var e = document.getElementById("overseas_data");
			var overseas_name = e.options[e.selectedIndex].text;
			
			if(std_overseas_id.length > 0 && contact != '' && ip != '' && overseas_name != '' && date != '' && time != '') {
				...
				axios({
						  method: "post",
						  url: "sendContact",
						  data: "std_overseas_id="+JSON.stringify(std_overseas_id)+"&ip="+ip+"&contact="+contact+"&overseas_name="+overseas_name+"&date="+date+"&time="+time,
						}).then(function (response) {
