...
$(function() {
	...
	axios({
		  method: "post",
		  url: "studentRequestDataForRole3",
			...
			$.each(response.data, function(i, data) {
				    ...
				    newCell6.innerHTML = "<button onClick='showModal(\""+response.data[i].user_id+"\",\""+ response.data[i].firstname+"\",\""+ response.data[i].lastname+"\",\""+ response.data[i].address+"\",\""+response.data[i].email+"\",\""+response.data[i].phone+"\",\""+response.data[i].profile_image+"\",\""+response.data[i].birthday+"\",\""+response.data[i].university_name+"\",\""+response.data[i].faculty_name+"\",\""+response.data[i].course_name+"\",\""+response.data[i].time_reg+"\",\""+response.data[i].overseas_name+"\",\""+response.data[i].gender+"\",\""+response.data[i].status+"\",\""+response.data[i].student_overseas_work_place_id+"\")' class='btn btn-primary' data-bs-toggle='modal' data-bs-target='#sendnotify'>รายละเอียด</button>"
				    newCell7.innerHTML = "<button onClick='manegementStatus("+response.data[i].user_id+", 1, "+response.data[i].student_overseas_work_place_id+")' class='btn btn-success'>ยืนยัน</button><button onClick='manegementStatus("+response.data[i].user_id+", 0, "+response.data[i].student_overseas_work_place_id+")' class='btn btn-danger'>ปฏิเสธ</button>"
	...
	function showModal(user_id, firstname, lastname, address, email, phone, profile_image, birthday, university_name, faculty_name, course_name, time_reg, overseas_name, gender, status, student_overseas_work_place_id) {
		...
		document.getElementById('overseaName').value = overseas_name
		document.getElementById('userStatus').value = status
		document.getElementById('overseaTimeReg').value = time_reg
		document.getElementById('userAddress').value = address
		document.getElementById('userId').value = user_id
		document.getElementById('userOverseasId').value = student_overseas_work_place_id
	}
	
	function manegementStatus(user_id, status, user_oversea_id) {
		...
		const data = $.getJSON('https://api.ipify.org?format=json', function(data){
			...
			if(user_id != '' && ip != '' && user_oversea_id != '') {
					
				axios({
					  method: "post",
					  url: "managementUserAndOverseasStatus",
					  data: 'user_id=' + user_id +'&status=' +status +'&ip='+ip+'&user_overseas_id='+user_oversea_id,
					}).then(function (response) {
						var icon = ""
